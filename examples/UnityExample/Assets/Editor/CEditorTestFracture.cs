...
public class CEditorTestFracture : EditorWindow
{
    ...
    private void _createPreview(bool makePrefab)
    {
        ...
        NvBlastAuthoringResult result = fractureTool.processFracture();
        ...
        List<BreakableChunk> chunks = new List<BreakableChunk>();
        ...
        for (int i = 1; i < fractureTool.getChunkCount(); i++)
        {
            ...
            BreakableChunk chunk = ck.AddComponent<BreakableChunk>();
            //chunk.Initialize(result.GetChunkDesc(i)); // -- do this later, when the breakable is set up
            chunks.Add(chunk);
        ...
        Breakable breakable = cs.AddComponent<Breakable>();
        //breakable.Initialize(result, chunks.ToArray());
